...
fetch(API_URL)
  ...
  .then(array => {
    completeArray = array
    const upcomingEvents = completeArray.events.filter(event => event.date >= completeArray.currentDate)
    renderPastCards(upcomingEvents)
    renderCheckBoxes(upcomingEvents)
  })
